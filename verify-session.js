...
export default async function handler(req, res) {
  ...
  try {
    const upstream = await fetch(
      `https://api.stripe.com/v1/checkout/sessions/${encodeURIComponent(sessionId)}?expand[]=subscription&expand[]=line_items.data.price.product`,
    ...
    // Como los pagos se hacen con Payment Links directos de Stripe (no con
    // create-checkout-session.js), no siempre llega metadata con el plan.
    // Por eso detectamos el plan mirando el NOMBRE del producto comprado —
    // mientras el producto en Stripe siga llamándose igual, esto no se rompe.
    ...
    if (!plan) {
      try {
        ...
        const productName = lineItem && lineItem.price && lineItem.price.product && lineItem.price.product.name;
    ...
    // Si ya existía un registro previo (por dispositivo o por email — ej. una
    // renovación, o esta misma persona entrando desde otro celular), respetamos
    // la fecha de inicio original — el "Día 1" no se reinicia solo porque pague de nuevo.
    const existingRaw = (deviceId && await kvGet(`access:${deviceId}`))
      || (email && await kvGet(`access_email:${email.toLowerCase()}`));
    if (existingRaw) {
      try {
    ...
    if (deviceId) await kvSet(`access:${deviceId}`, record);
    // Este es el índice clave: permite que la misma persona recupere su
    // acceso en cualquier otro dispositivo con solo escribir su email.
    if (email) await kvSet(`access_email:${email.toLowerCase()}`, record);
